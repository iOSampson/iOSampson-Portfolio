...
function ryancv_scripts() {
	...
	if ( $scroll_type ) {
		wp_enqueue_script( 'sticky-kit', get_template_directory_uri() . '/assets/js/sticky-kit.min.js', array('jquery'), '1.0.0', true );
	}
...
function ryancv_wp_nav_menu_objects( $items, $args ) {
	...
	foreach( $items as &$item ) {
		...
		// append icon
		if( $icon ) {
			$icon_class = str_replace('<i class="', '', $icon); 
			$icon_class = str_replace('"></i>', '', $icon_class);
			$item->title = '<span class="icon ' . $icon_class . '"></span><span class="name">' . $item->title . '</span>';
		}
	}
	// return
	return $items;
