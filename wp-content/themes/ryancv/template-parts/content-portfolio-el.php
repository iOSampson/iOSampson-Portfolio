...
<?php
	...
	if ( $type == 2 ) {
		...
		$popup_url_target = get_field( 'link_url_target', $id );
		$popup_link_target = $popup_url_target;
	...
	<div class="box-item">
		<div class="image">
			<?php if ( $portfolio_qv ) : ?>
				...
				<?php endif; ?>
			<?php else : ?>
				<a href="<?php echo esc_url( $popup_url ); ?>" class="<?php echo esc_attr( $popup_class ); ?>"<?php if ( $popup_link_target == 0 ) : ?> target="_blank"<?php endif; ?><?php if ( $popup_elementor == 0 ) : ?> data-elementor-lightbox-title="<?php echo esc_attr( $title ); ?>" data-elementor-lightbox-slideshow="gallery-<?php echo esc_attr( $id ); ?>"<?php endif; ?>>
					<?php if ( has_post_thumbnail( $id ) ) : 
						echo get_the_post_thumbnail( $id, 'ryancv_600xauto' );
					endif; ?>
					<span class="info">
